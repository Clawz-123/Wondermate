...
const ManageTravelPackages = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const upLoadData = async () => {
      try {
        const response = await axios.post("http://localhost:5269/api/TravelPackage",
           travelPackagesData);
    ...
    const editData = async () => {
      try {
        const response = await axios.put(`http://localhost:5269/api/TravelPackage/${id}`, travelPackagesData);
        console.log(response);
        resetForm();  
      } catch (err) {
        console.log(err);
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:5269/api/TravelPackage/${id}`);
      console.log(response);
      fetchTravelPackage();
    } catch (err) {
      console.log(err);
  ...
  const fetchTravelPackage = async () => {
    try {
      const response = await axios.get("http://localhost:5269/api/TravelPackage");
      console.log(response)
      setTravelPackageData(response.data);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  return (
    <>
      <div className="w-full p-5 shadow-xl drop-shadow-sm flex flex-col mx-5 my-5">
        <h1 className="font-bold mb-2 text-2xl">{isEditing ? "Edit TravelPackage" : "Add New TravelPackage"}</h1>
        <form onSubmit={(e) => handleSubmit(e)}>
          ...
          <button
            type="submit"
            className={`bg-sky-500 h-10 w-[23vh] rounded-md mb-10 text-white ${loading ? "cursor-not-allowed bg-blue-300" : "cursor-pointer"}`}
            disabled={loading}
          >
            {isEditing ? "Update TravelPackage" : "Add TravelPackage"}
          </button>
        </form>

        <div>
          <h1 className="font-bold mb-10 text-2xl">View TravelPackages</h1>
          <table className="w-full">
            ...
            <tbody className="border-collapse divide-y border-b-4">
              {TravelPackageData.map((TravelPackage) => (
                <tr className="border" key={TravelPackage.id}>
                  <td className="border-b-2 px-4 py-2">{TravelPackage.name}</td>
                  <td className="border px-4 py-2">{TravelPackage.price}</td>
                  <td className="border px-4 py-2">
                    <button className="bg-green-500 rounded-md text-white h-6 w-20" onClick={() => handleEdit(TravelPackage)}>
                      Edit
                    </button>
                  </td>
                  <td className="border px-4 py-2">
